...
import { 
  ...
  Upload, Trash2, Brain, Copy, Languages, MapPin, Plus, Send
} from 'lucide-react'
...
export default function Home() {
  ...
  const [expandedLegalCards, setExpandedLegalCards] = useState<number[]>([])
  ...
  const [sosLocationLink, setSosLocationLink] = useState('')
  const [supportAmount, setSupportAmount] = useState('100')
  const [supportContributorName, setSupportContributorName] = useState('')
  const [supportStatus, setSupportStatus] = useState('')
  const [supportUpiLink, setSupportUpiLink] = useState('')
  const [supportQrCodeUrl, setSupportQrCodeUrl] = useState('')
  ...
  const [isSubmittingAdvertise, setIsSubmittingAdvertise] = useState(false)
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('justiceai_trusted_contacts')
      if (!raw) return
      const parsed = JSON.parse(raw)
      if (Array.isArray(parsed)) {
        setTrustedContacts(parsed.filter((c) => typeof c?.name === 'string' && typeof c?.phone === 'string'))
      }
    } catch {}
  }, [])

  useEffect(() => {
    localStorage.setItem('justiceai_trusted_contacts', JSON.stringify(trustedContacts))
  }, [trustedContacts])
  ...
  const addTrustedContact = () => {
    ...
    if (!name || phone.length < 10) {
      alert('Enter valid contact name and phone number')
      return
    }
  ...
  const toggleLegalCardDetails = (index: number) => {
    setExpandedLegalCards((prev) => (prev.includes(index) ? prev.filter((i) => i !== index) : [...prev, index]))
  }
  const triggerSOS = async () => {
    ...
    try {
      ...
      setSosLocationLink(mapsLink)
      setSosMessage(message)
      try {
        await fetch(`${API_BASE_URL}/cases/public/sos`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message, locationLink: mapsLink, latitude: lat, longitude: lon, trustedContacts }),
        })
      } catch {}
      try {
        await navigator.clipboard.writeText(message)
      } catch {}
      setSosStatus('SOS generated. Message copied. Use SMS/WhatsApp actions below.')
    } catch (error) {
      setSosStatus(error instanceof Error ? error.message : 'Unable to activate SOS')
    } finally {
  ...
  const createDonationIntent = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/monetization/public/donation-intent`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount: Math.round(amount),
          contributorName: supportContributorName || 'Supporter',
          purpose: 'JusticeAI platform sustainability',
        }),
      })
      const data = await response.json()
      if (!response.ok || !data.success) throw new Error(data.error || 'Failed to create payment intent')
      setSupportUpiLink(data.upiLink || '')
      setSupportQrCodeUrl(data.qrCodeUrl || '')
      setSupportReference(data.reference || '')
      setSupportStatus(`Payment request created for ₹${data.amount}. Tap "Open UPI App" or scan QR.`)
    } catch (error) {
      setSupportStatus(error instanceof Error ? error.message : 'Unable to start payment')
    }
  }
  const openUpiApp = () => {
    if (!supportUpiLink) {
    ...
    setSupportStatus('If UPI app did not open, scan QR code or copy payment link.')
  }
  const copySupportPaymentLink = async () => {
    if (!supportUpiLink) {
      alert('Generate a payment link first')
      return
    }
    try {
      await navigator.clipboard.writeText(supportUpiLink)
      alert('UPI payment link copied')
    } catch {
      alert('Unable to copy payment link')
    }
  }
  const submitAdvertiseInquiry = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsSubmittingAdvertise(true)
    setAdvertiseStatus('')
    try {
      ...
      setAdvertiseStatus(`Inquiry submitted. Reference: ${data.inquiryId}`)
      setAdvertiseForm({ name: '', email: '', organization: '', budget: '', message: '' })
    } catch (error) {
      setAdvertiseStatus(error instanceof Error ? error.message : 'Unable to submit inquiry')
    } finally {
      setIsSubmittingAdvertise(false)
    }
  }
  ...
  const legalAwarenessDetails: Record<string, string[]> = {
    'Know Your Rights': [
      'Right to equality and equal protection under law.',
      'Right to constitutional remedies when rights are violated.',
      'Right to legal aid in eligible matters.',
    ],
    'Legal Procedures': [
      'How to file complaint/FIR and keep acknowledgment.',
      'How to organize timeline and evidence.',
      'How to escalate if action is delayed.',
    ],
    'Court System': [
      'District courts, high courts, and supreme court roles.',
      'Civil vs criminal jurisdiction basics.',
      'Typical case lifecycle from filing to judgment.',
    ],
    'Evidence Collection': [
      'Keep original files and timestamped copies.',
      'Capture date, time, place, and witness context.',
      'Store screenshots, documents, and media safely.',
    ],
    'Legal Terminology': [
      'Meaning of FIR, bail, summons, affidavit, and charge sheet.',
      'Difference between complaint, petition, and appeal.',
      'How notices and replies are handled.',
    ],
    'Your Responsibilities': [
      'Provide truthful details and cooperate with lawful process.',
      'Avoid tampering evidence and keep records safe.',
      'Report emergencies immediately to 112/100.',
    ],
  }
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <button
                onClick={() => {
                  const el = document.getElementById('legal-awareness-section')
                  el?.scrollIntoView({ behavior: 'smooth' })
                }}
                className="text-gray-700 hover:text-blue-600 transition font-medium"
              >
              ...
              <button
                onClick={() => {
                  const el = document.getElementById('sustainability-section')
                  el?.scrollIntoView({ behavior: 'smooth' })
                }}
                className="text-gray-700 hover:text-blue-600 transition font-medium"
              >
      ...
      <section id="legal-awareness-section" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* SOS Alert Section */}
      <section id="sos-section" className="py-20 bg-red-50 border-y border-red-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <h2 className="text-4xl md:text-5xl font-bold text-red-700 mb-3">Emergency SOS Alert</h2>
            <p className="text-lg text-red-900">One-click emergency alert with live location sharing.</p>
          </div>
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-lg border border-red-200 p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-3">Activate SOS</h3>
              <button
                type="button"
                onClick={triggerSOS}
                disabled={isSOSLoading}
                className="w-full rounded-lg bg-red-600 px-5 py-4 text-lg font-bold text-white hover:bg-red-700 disabled:bg-red-300"
              >
                {isSOSLoading ? 'Activating SOS...' : '🚨 Activate SOS'}
              </button>
              <div className="mt-4 grid grid-cols-2 gap-3">
                <a href="tel:112" className="rounded-lg bg-gray-900 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-black">Call 112</a>
                <a href="tel:100" className="rounded-lg bg-blue-600 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-blue-700">Call 100</a>
              </div>
              {sosStatus && <p className="mt-4 rounded-lg bg-red-100 p-3 text-sm text-red-800">{sosStatus}</p>}
              {sosLocationLink && (
                <a href={sosLocationLink} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-blue-700 hover:text-blue-900">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-3">Trusted Contacts</h3>
              <div className="grid sm:grid-cols-2 gap-3">
                <input type="text" value={newContactName} onChange={(e) => setNewContactName(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" placeholder="Contact name" />
                <input type="text" value={newContactPhone} onChange={(e) => setNewContactPhone(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" placeholder="Phone number" />
              </div>
              <button type="button" onClick={addTrustedContact} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-black">
                <Plus className="w-4 h-4" />
                Add Contact
              </button>
              <div className="mt-4 space-y-3">
                {trustedContacts.length === 0 && <p className="text-sm text-gray-500">No trusted contacts added yet.</p>}
                {trustedContacts.map((contact, index) => (
                  <div key={`${contact.phone}-${index}`} className="rounded-lg border border-gray-200 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-gray-900">{contact.name}</p>
                        <p className="text-sm text-gray-600">{contact.phone}</p>
                      </div>
                      <button type="button" onClick={() => removeTrustedContact(index)} className="text-xs font-semibold text-red-600 hover:text-red-700">
                        Remove
                      </button>
                    </div>
                    {sosMessage && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        <a href={`sms:${formatPhoneForCall(contact.phone)}?body=${encodeURIComponent(sosMessage)}`} className="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">SMS</a>
                        <a href={`https://wa.me/${formatPhoneForWhatsApp(contact.phone)}?text=${encodeURIComponent(sosMessage)}`} target="_blank" rel="noreferrer" className="rounded-md bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700">WhatsApp</a>
                        <a href={`tel:${formatPhoneForCall(contact.phone)}`} className="rounded-md bg-gray-800 px-3 py-1.5 text-xs font-semibold text-white hover:bg-black">Call</a>
                      </div>
                    )}
                  </div>
                ))}
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      alert('Unable to copy SOS message')
                    }
                  }}
      ...
      {/* Sustainability & Advertising Section */}
      <section id="sustainability-section" className="py-20 bg-white border-t border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Support & Promote JusticeAI</h2>
            <p className="text-xl text-gray-600">Minimum ₹100 support payment and advertising inquiries.</p>
          </div>
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-6">
              <h3 className="text-2xl font-bold text-gray-900 mb-2">Minimum Support Payment</h3>
              <div className="space-y-3">
                <input type="text" value={supportContributorName} onChange={(e) => setSupportContributorName(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Your name (optional)" />
                <input type="number" min={100} value={supportAmount} onChange={(e) => setSupportAmount(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Amount in INR (minimum 100)" />
              </div>
              <div className="mt-4 flex flex-wrap gap-3">
                <button type="button" onClick={createDonationIntent} className="rounded-lg bg-blue-600 px-5 py-3 text-sm font-bold text-white hover:bg-blue-700">Generate Payment Link</button>
                <button type="button" onClick={openUpiApp} className="rounded-lg bg-green-600 px-5 py-3 text-sm font-bold text-white hover:bg-green-700">Open UPI App</button>
                <button type="button" onClick={copySupportPaymentLink} className="rounded-lg bg-gray-900 px-5 py-3 text-sm font-bold text-white hover:bg-black">Copy UPI Link</button>
              </div>
              {supportQrCodeUrl && (
                <div className="mt-4 rounded-lg border border-gray-200 bg-white p-4">
                  <p className="text-sm font-semibold text-gray-800 mb-2">UPI QR (fallback)</p>
                  <img src={supportQrCodeUrl} alt="UPI payment QR" className="h-44 w-44 rounded border border-gray-200" />
                </div>
              )}
              {supportReference && <p className="mt-3 text-sm text-gray-700">Reference ID: {supportReference}</p>}
              {supportStatus && <p className="mt-2 text-sm text-blue-700">{supportStatus}</p>}
            </div>
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-6">
              <h3 className="text-2xl font-bold text-gray-900 mb-2">Advertise / Partnership Inquiry</h3>
              <form onSubmit={submitAdvertiseInquiry} className="space-y-3">
                <input type="text" required value={advertiseForm.name} onChange={(e) => setAdvertiseForm({ ...advertiseForm, name: e.target.value })} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Full name" />
                <input type="email" required value={advertiseForm.email} onChange={(e) => setAdvertiseForm({ ...advertiseForm, email: e.target.value })} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Email address" />
                <input type="text" required value={advertiseForm.organization} onChange={(e) => setAdvertiseForm({ ...advertiseForm, organization: e.target.value })} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Organization / Brand" />
                <input type="text" value={advertiseForm.budget} onChange={(e) => setAdvertiseForm({ ...advertiseForm, budget: e.target.value })} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="Campaign budget (optional)" />
                <textarea required rows={4} value={advertiseForm.message} onChange={(e) => setAdvertiseForm({ ...advertiseForm, message: e.target.value })} className="w-full rounded-lg border border-gray-300 px-4 py-3" placeholder="What would you like to advertise and target audience?" />
                <button type="submit" disabled={isSubmittingAdvertise} className="w-full rounded-lg bg-blue-600 px-5 py-3 text-sm font-bold text-white hover:bg-blue-700 disabled:bg-blue-300">
                  {isSubmittingAdvertise ? 'Submitting...' : 'Submit Inquiry'}
                </button>
              </form>
              {advertiseStatus && <p className="mt-3 text-sm text-blue-700">{advertiseStatus}</p>}
            </div>
          </div>
        </div>
      </section>
